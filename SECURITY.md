...
报告应包含受影响提交或版本、操作系统版本（Linux 请注明发行版、桌面和 X11/Wayland 会话）、最小复现、预期影响和使用虚构数据的示例。普通取词兼容问题请使用缺陷模板。

## 安全边界

渲染进程不具有 Node 权限；主进程验证 IPC 来源并管理凭据与网络请求；模型输出按纯文本显示。API Key 通过 Electron safeStorage 加密保存：Windows 使用系统保护，Linux 要求可用的系统密钥环并拒绝 `basic_text` 后端。这不能抵御已控制当前用户会话的恶意程序。

Linux 拒绝 `--no-sandbox` 启动，包括 AppImage 启动器自动添加该参数的情况。请配置发行版支持的 Chromium 沙箱环境，不要通过关闭沙箱来绕过启动失败。
