...
所有「指令」动作都提供 **记录** 按钮，包括翻译、解释、润色和自定义动作。点击后，把原文、结果和来源进程保存在各自的本地数据库中；同一卡片重试后再次记录，会更新已有结果。在设置的「历史」页按动作切换，可按时间浏览记录、查看原文和结果，并分别复制。停用动作不影响查看其历史。历史浏览和自定义动作记录从 0.3.0 起提供，暂不支持搜索。
